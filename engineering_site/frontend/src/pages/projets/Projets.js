...
function Projets() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      apiFetch(`projets?id_employe=${user.id_employe}&role=${user.role || ""}`)
  ...
  const deleteProjet = (id) => {
    ...
    apiFetch(`projets/${id}`, { method: "DELETE" })
    ...
    <div className="ms-root">
      <div className="ms-page">
        <div className="ms-page-header">
          ...
          <Link to="/create-projet" className="ms-btn-new">+ Nouveau Projet</Link>
        </div>

        <div className="ms-stats" style={{ gridTemplateColumns: "repeat(4,1fr)" }}>
          <div className="ms-stat-card stat-total">
            <div><span className="ms-stat-val">{stats.total}</span><span className="ms-stat-label">Total Projets</span></div>
          </div>
          ...
          <div className="ms-stat-card stat-suspendu">
            <div><span className="ms-stat-val">{stats.suspendu}</span><span className="ms-stat-label">Suspendus</span></div>
          </div>
        </div>
        ...
        <div className="ms-table-wrap">
            ...
            paginated.map(p => (
              <div key={p.id_projet} className="ms-table-row" style={{ gridTemplateColumns: COLS }}>
                ...
                <div className="ms-cell-actions">
                  <Link to={`/edit-projet/${p.id_projet}`} className="ms-btn-edit">Modifier</Link>
                  <button className="ms-btn-edit red" onClick={() => deleteProjet(p.id_projet)}>Supprimer</button>
                </div>
              </div>
            ))
          )}
          ...
          {filtered.length > PAGE_SIZE && (
            <div className="ms-pagination">
              <span className="ms-page-info">Page {page} sur {totalPages} — {filtered.length} projets</span>
        ...
        <div onClick={() => setSelectedEmp(null)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ textAlign: "right", marginTop: 16 }}>
              <button className="ms-btn-edit red" onClick={() => setSelectedEmp(null)}>Fermer</button>
            </div>
          </div>
        </div>
      )}
